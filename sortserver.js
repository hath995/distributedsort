...
configData = config.data[id];

value = configData.value;
leftNeighbor = configData.left;
rightNeighbor = configData.right;

function getLeftNeighbor() {
    return ( leftNeighbor ) ? leftNeighbor : null;
}

function getRightNeighbor() {
    return ( rightNeighbor ) ? rightNeighbor : null;
}
...
var initializationInterval = setInterval(function() {
        send(function(err) {
                if (!err) {
                    clearInterval(intializationInterval);
                }
            });
    });